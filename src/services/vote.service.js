...
const CreatePolls = async( PollsBody ) => {
    ...
    const Polls = database.query(`INSERT INTO polls( userid ,title, isLock, createdAt ) VALUES( ?, ?, ?, ? )`, [ PollsBody.userid, PollsBody.title, PollsBody.isLock, PollsBody.createdAt ]);
    if( Polls[0].length === 0 ) {
        console.log( Polls[0] );
...
const DeletePoll = async( PollID ) => {
    try {
        ...
        const _RemoveOption = await RemoveOption( PollID );
        console.log( _RemoveOption )
        if ( !_RemoveOption.status ) {
            return { message : "DELETE USER OPTION FAIL", status: false  };
        };

        const DeletePoll = await database.query(`DELETE FROM polls WHERE id = ?`, [ PollID ]);
        if( DeletePoll[0].length === 0 ) {
...
const RemoveOption = async( OptionID ) => {
    try {
        ...
        if( isExistUserOption[0].length !== 0 ) {
            ...
            if( RemoveUserOption[0].length === 0 ) {
